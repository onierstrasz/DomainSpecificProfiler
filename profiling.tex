...
The advances of domain-specific language and modeling reveals a drastic change in the way software is being built. Models are becoming first class entities, and so, to better define software application domain. The software engineering community has seen a rapid emergence of domain-specific tools, ranging from tools to easily build domain-specific languages~\cite{Viss04a} to language checking and embedding tools~\cite{Reng10a}.
...
Whereas research on domain-specific languages made consistent progress on language specification, implementation and verification, little has been done on profiling.

This paper gives the essential property an effective domain-specific profiler must meet. 
It is important to notice that, as models are becoming first class entities, a profiler's measurement dimension cannot be solely limited to execution time.
%Note that we take a rather large definition of profiler by not solely focusing on a timely execution. 
We consider profiling as the activity \ugh{to record recording} \jr{is this correct Alex?} and analysis of a program execution. 
Profiling is often considered essential when one wants to obtain the representation of a program execution. \jr{I do not know about this sentence, because it implies that there is no runtime we can analyze, which is not true. I think that we should say something like: Profiling is essential to analyze complex data that otherwise would be to hard to harvest and compare by normal debugging.}
Profiling is essential to analyze complex data that otherwise would be to hard to harvest and compare by normal debugging.
...
The profiling of Mondrian is visualized using Mondrian itself. The following {\sf \#visualize} method describes the visualization given in \autoref{fig:MondrianProfiler}.

\begin{lstlisting}
MSMondrianSpy>>visualize
	| view types |
	view := MOViewRenderer title: 'Profiler'.
...
The important point of {\sf \#visualize} is to regularly update the visualization, complying with the \emph{Profile evolution} property. The profiler is uninstalled when the profiler Mondrian visualization is closed.
...
There are two ways of implementing an observer: registering the observer with preexistent event-based systems, or using the meta-level programming techniques of the host language.

Lets consider the Announcement Observer, whose responsibility is to observe the generation of specific announcements.
...
<b>AnnouncementObserver>>install</b>
	self announcer 
			...
			to: self handler
\end{lstlisting}
The method \ct{install} is responsible for installing the observer. In the previous snippet of code we can see that the observer is hook to the announcement system by evaluating the observer's handler when an announcement is triggered.
...
In The method \ct{setUp} initializes the profiling meta-object with a behavioral meta-object. 
This meta-object will evaluate the handler when an specific message is received by the profiled object alone. This is called object-specific instrumentation.

Object-specific instrumentation is hard to achieve in class-based languages like Smalltalk and Java. 
...
The design of Albedo can be understood as an evolution of partial behavioral reflection for Smalltalk, which in turn was conceived as an extension of the Reflex~\cite{Tant03a} model of \emph{Partial Behavioral Reflection}.
Since Reflex was originally realized with Java, the Albedo approach would be achievable in a more static mainstream language like Java. 
