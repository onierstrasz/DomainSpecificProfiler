...
%\lr{do not understand:} The need to better define software application domain favored metamodels to be considered as first class-entities. 
The software engineering community has seen a rapid emergence of domain-specific tools, ranging from tools to easily build domain-specific languages~\cite{Viss04a}, transform models~\cite{Tisi10a} and language checking \lr{What is `language checking'? There is only `program checking'} and embedding tools~\cite{Reng10a} \lr{The ECOOP paper does not talk about `program checking', only \cite{Reng10b}}. \jr{@Lukas can you fix this, I do not fully understand your comment.}
...
While research on domain-specific languages made consistent progress on language specification \cite{Deur00a}, implementation and verification, little has been done on profiling. \ab{We need to strengthen this paragraph with some references}
...
\jr{we have to clarify this sentence:} The proliferation of meta-models and domain-specific languages \chg{builds}{brought?} new abstractions that map to the underlying execution platform in non-trivial ways. Traditional profiling tools fail to display relevant information in the presence of such abstractions.
...
%\lr{that has nothing to do with our paper:} visualizing an enriched dependency structural matrix~\cite{Lava09a}. 
...
This performance issue has been solved a few days after the complain via a quantification of the displayOn: invocation on each objects and metric computation.
...
The information collected by traditional profilers is textual targeting method invocation. A method that invokes another will be located below it and indented. Moreover each method frame represented has a class name and a method name, which completely ignores the identity of the object and arguments that are part of the call.

Collected information have to be presented in a fashion so that the important metrics and domain object composition are on the foreground.  
...
\jr{@Alex @Lukas I think that these are the solutions that we provide to solve the requirements previously mentioned. So I would suggest removing these points from here and stressing them in the approach and in the use cases.}
