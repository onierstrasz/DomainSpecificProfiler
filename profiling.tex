...
\newcommand{\Authors}{Alexandre Bergel$^1$, Lukas Renggli, Jorge Ressia$^2$, Oscar Nierstrasz$^2$}
\newcommand{\AuthorsShort}{A. Bergel, L. Renggli, J. Ressia, O. Nierstrasz}
...
%\ab{What is necessary to instantiate MetaSpy does not directly map to the requirements we identified. This may be a problem on some point}
...
Recent advances in domain-specific languages and models reveal a drastic change in the way software is being built. The software engineering community has seen a rapid emergence of domain-specific tools, ranging from tools to easily build domain-specific languages~\cite{Viss04a}, to transform models~\cite{Tisi10a}, to check source code~\cite{Reng10b}, and to integrate development tools~\cite{Reng10a}.

While research on domain-specific languages has made consistent progress in language specification~\cite{Deur00a}, implementation~\cite{Cuad09a}, evolution~\cite{Free06a} and verification~\cite{Kaba08a}, little has been done to support profiling. We consider profiling to be the activity of recording and analyzing program execution. Profiling is essential for analyzing transient run-time data that otherwise would be too hard to harvest and compare. Code profilers commonly employ execution sampling as the way to obtain dynamic run-time information. Unfortunately, information extracted by regularly sampling the call stack cannot be meaningfully used to profile a high-level domain built on top of the standard language infrastructure. Specialized domains need specialized profilers.
%\on{Not really a crisp statement of the problem. The point is, I think, that sampling-based profilers focus on certain kind of information which is not really what you are looking for in certain domains. We need other, specialized profilers that dig up other kinds of information. Mondrian gives an example of different kind of profiling information needed. The question then is, can we have a framework for easily building specialized profilers. The title of the paper expresses this beautifully, but the above paragraph fails to home in on the key point. I would just add the following sentence to round out the paragraph:} \ab{I absolutely agree with you}

%\lr{I don't really see how Mondrian demonstrates the problem. I would outline the problem as given in the commented bullet list at the beginning of this section.}

Let us consider the example of the Mondrian visualization engine (details follow in \autoref{mondrian}).
Mondrian models visualizations as graphs, \ie in terms of nodes and edges. One of the important performance issues we recently faced is the refresh frequency: nodes and edges were refreshed too often.
...
Standard code profilers did not help to localize the source of the problem since they are just able to report the share of time the CPU spent in the method \co{\#displayOn:} of the class \co{MONode} and \co{MOEdge}.
The problem was finally resolved by developing a custom profiler that could identify \emph{which} nodes and edges were indeed refreshed too often. This domain-specific profiler was able to exploit knowledge of Mondrian's domain concepts to gather and present the needed information.

% Knowing the presence of some particular frames on the method call stack (as the majority of code profiler provide) does not help identify which of the rendered nodes are responsible for a slowdown. The Mondrian domain is defined as a graph of objects, which cannot be easily retrieved from the method call stack and from the source code artifacts. A specialized Mondrian profiler would be aware of Mondrian's domain concepts and provide profiling information tailored to the relevant structures.
%The Mondrian domain particularities \lr{what are these?} are superfluous by standard code profiler which are only concerned with stack frames and source code artifacts.
...
We argue that there is a need for a general approach to easily develop specialized profilers for domain-specific languages and tools.
A general approach must offer means to (i) \emph{specify} the domain concepts of interest, (ii) \emph{capture} the relevant information from the run-time execution, and (iii) \emph{present} the results to the developer.

In this paper we introduce such a general approach for domain-specific profiling based on events. In this approach, called \project, one specifies the events of interest for a given domain, one captures them either by subscribing to existing application events, or to events generated with the help of a reflective layer, and one presents them as a graph-based visualization.
...
%%\ugh{\emph{specifying the domain}, \emph{capture domain events}, \emph{effective information presentation}}. \on{should be: specifying, capturing, presenting}
...
\paragraph{Outline.} 
The remainder of this paper is structured as follows: \autoref{sec:problem} illustrates the problems of using a general-purpose profiler on code that is built on top of domain-specific language. \autoref{sec:profiler} introduces our approach to domain-specific profiling.
\autoref{sec:validation} demonstrates how our approach solves the requirements of domain-specific profilers with three use cases. \autoref{sec:implementation} presents our infrastructure to implement domain-specific profilers; and \autoref{sec:conclusion} summarizes the paper and discusses future work.
...
%The profiler produces a textual output, \on{no. do not use a comma to join two sentences in English!} we next show an \ugh{except} \on{?} of it: \on{crappy sentence. rewrite}
...
It would be useful to establish how much of the grammar is actually exercised by a set of test files in order to identify unused and duplicated parsing rules.
The \ct{if} statement parsing rule is defined as follows\footnotetext{Readers unfamiliar with the syntax of Smalltalk might want to read the code examples aloud and interpret them as normal sentences: An invocation to a method named \ct{method:with:}, using two arguments looks like: \ct{receiver method: arg1 with: arg2}. The semicolon separates cascaded messages that are sent to the same receiver. For example, \ct{receiver method1: arg1; method2: arg2} sends the messages \ct{method1:} and \ct{method2:} to \ct{receiver}. Other syntactic elements of Smalltalk are: the dot to separate statements: \ct{statement1. statement2}; square brackets to denote code blocks or anonymous functions: \ct{[ statements ]}; and single quotes to delimit strings: \ct{'a string'}. The caret \ct{^} returns the result of the following expression.}:
...
Coverage tools assess the coverage of the application source code by listing the methods involved in an execution. Some tools can even detect the coverage inside methods.
Let's consider the Java grammar\footnote{\url{http://www.squeaksource.com/PetitJava.html}} for PetitParser which is defined with 210 rules. These rules are modeled by a graph of objects. 
Traditional coverage tools focus on the source code artifacts instead of domain-specific data. That is why we are unable to analyze the parsing and production coverage of the Java Grammar. 
%When analyzing the parsing and production coverage of the Java Grammar parser, 
%Traditional coverage tools focus on certain kind of information which is not really what you are looking for in certain domains

%When in need of analyzing the parsing and production coverage of the Java Grammar parser traditional coverage tools only provide source code related data instead of domain-specific.} \on{puke! The subject of the sentence is "coverage tools", which are not in need of anything! rewrite}
...
In this section we will present \project, a framework to easily build domain-specific profilers. 
The key idea behind \project is to define domain-specific events that can later be used by different profilers with different objectives.
...
An observer is responsible for adapting a domain-specific model and triggering specific actions when certain events happen.
These observers follow the observer pattern~\cite{Gamm95a}.
A profilers models a domain-specific profiling requirement through composing multiple observers.

%\on{Ugh! You are confusing different terms for the same thing. In the observer pattern there are observers and subjects. Synonyms are subscribers (observers) and publishers (subjects). In this diagram it appears that the profilers are not subscribers (observers) at all, nor are they subjects (publishers), but rather they manage a list of observers. Better get your story right!}
...
In this section we will analyze three case studies from completely different domains.
We will show how \project is useful for expressing the different profiling requirements in terms of events.
We will also demonstrate how \project fulfills the domain-specific profiling requirements: specifying, capturing and presenting domain-specific information.

%:========
\subsection{Case Study: Displaying invocations}
...
\subsection*{Capturing the domain}
%\paragraph{Capturing the domain}
...
The instrumentation described in the {\sf setUp} method is only applied to the model specified in the profiler.
This model is an object which models the domain to be profiled, in this case a Mondrian visualization.
The instrumentation is only applied to all nodes in this visualization. Only when these nodes receive the the message {\sf \#displayOn:} will increment the actual counter.
This object-specific behavior is possible due to the use of a reflection framework called Albedo.


\subsection*{Presenting the domain}
...
% \ab{I would remove this listing actually. It does not give anything new}
% \begin{lstlisting}
...
The profiling monitors each node of the profiled visualization. Each node is associated to a progress bar that widens upon node refresh. The profiled visualization remains interactive. Clicking and drag-and-dropping nodes refresh the visualization, thus increasing the progress bar of the corresponding nodes. This profile helps identifying unnecessary rendering. We identified a situation in which nodes were refreshing without receiving user actions. This was perceived by the user with a sluggish rendering. Edges were constantly refreshed, even without being apparent. This problem is addressed in the version 2.30 of Mondrian\footnote{\url{http://code.google.com/p/moose-technology/issues/detail?id=450}}. 

%\on{not sure what we learn from this case study. the reader gets a bit lost in the details. at least provide a short summary paragraph underlining what we have learned, and how this validates \project. aside from the fact that we have now a domain-specific profile for mondrian, what is cool?} \ab{humm... I get stuck on this. This profiler helped solving the problem described in Section 2.1.}
...
% 
% The performance issue of Mondrian described earlier (\autoref{sec:problem}) is rooted in a complex combination of different aspects of Mondrian rendering: redundancy of metrics calculation and unnecessary repainting of graphical elements. Theses two aspects were independently addressed, using a dedicated profiler \ins{for each}.
% \on{Funny. In the intro you just said "without a standard profiler". You should be more precise in the intro.}\ab{I fixed the intro}
% 
% \paragraph{Keeping track of the cache activations.}
% Each node has a number of caches. Activation of theses caches is governed by complex rules which depend on what is currently displayed, the nesting of nodes and their size. Incorrect implementation of the caches can be a source of hard-to-track bugs and abnormal behavior. Mondrian offers two kinds of caches:
% \begin{itemize}
% \item A node may nest other nodes and edges. The \emph{bitmap form cache} avoids rendering inner nodes by taking a snapshot of a bitmap of the nested node. Once the bitmap cache is loaded, inner nodes are not rendered any further.
% \item The shape of a graphical element is the result of metric calculation on the model object represented by the element. Each dimension of a graphical element is mapped to a metric. The \emph{metric cache} avoids the need to recompute the metrics at each rendering.
% \end{itemize}
% 
% Keeping track of the activation and deactivation of these two caches is realized with a dedicated profiler, called {\sc MondrianCacheActivation}. The profiler is implemented as a class \co{MSMondrianActivationCacheSpy}. The class inherits from \co{MSAbstractMondrianSpy}, which will also be used for the second profiler.
% 
...
% \end{lstlisting}
% 
% \lr{Why is this better than what the Squeak damage visualizer and the OS X GUI damage visualizer provides?}\ab{I do not know about the squeak and OSX damage visualizer. How can I invoke it? I tried some googling but found nothing}\lr{Look for \emph{morphic damage} in the settings, the setting used to be called something like that (I don't have a Pharo image at hand). Enabling it would highlight all Morphic updates quickly after drawing them. I used it for the original Mondrian implementation.}\ab{It works well. I did not know about that} \lr{OS X has essentially the same in the Developer Tools, but again I don't have access to that right now. There is GUI debugging utility that can visualize different kinds of events on the live screen in various colors.} \ab{I am not sure whether we can mention it in the paper, I haven't found a compelling url about it. Just http://developer.apple.com/library/mac/\#documentation/Performance/Conceptual/Drawing/Articles/FlushingContent.html}
% 
% {\sc MondrianCacheActivation} instantiates \project in less than 22 lines of code. 
% The code opens a new Mondrian visualization that visualizes the profiling the nodes of another visualization, given by an \co{nodes} instances variable. The \emph{domain} is the nodes of the profiled visualization. Lines 7 and 12 describes the following steps: nodes are displayed and the ownership (nesting) is represented with edges. The domain is profiled against two relevant properties. These are represented in Lines 14 and 17: a bitmap form and metrics. The profile is presented via the Mondrian script given above. The evolution over execution is realized via a periodical refresh, indicated in the script at Line 11. The model being profiled and the profiler are represented with different objects in the host language. The scope of the profiling is naturally given by operating on a different object.
% 
...
OmniBrowser~\cite{Berg08c} is a framework to define and compose new browsers: graphical list-oriented tools to navigate and edit elements from an arbitrary domain. In the OmniBrowser framework, a browser is described by a domain model specifying the domain elements that can be navigated and edited, and a metagraph specifying the navigation between these domain elements. Nodes in the metagraph describe states the browser is in, while edges express navigation possibilities between those states. The OmniBrowser framework then dynamically composes widgets such as list menus and text panes to build an interactive browser that follows the navigation described in the metagraph.

OmniBrowser uses announcements for modeling the interaction events of the user with the IDE.
A very common problem is to have certain announcements to be triggered two many times for certain scenarios.
This behavior impacts negatively in the performance of the IDE. Moreover, is some cases strange displaying problems are produced which are very hard to track down.


%\lr{What does this show?}

\subsection*{Capturing the domain}
To profile this domain-specific case we implemented the class \co{\sf MSOmniBrowserSpy}:
...
The instrumentation in the {\sf setUp} method counts how many times each announcement was triggered.
...
\subsection*{Specifying the domain}
We specify the entities we are interested in profiling by defining the model in the profiler.
For example, we could define \co{\sf OBSystemBrowser} browsing an specific class.
All Omni Browser have an internal collaborator named \emph{announcer} which is responsible for the signaling of announcements.
This is the object used by the profiler to catch the announcement events.

\subsection*{Presenting the domain}
A Mondrian visualization was implemented to list the type and the number of announcement triggered.
...
Line~2 iterates over all primitive parser objects in the grammar. Line~3 attaches the event handler on Line~4 to each parser in the model. The handler then counts the activations of each parser object when we run the test suite of the grammar. 
...
\subsection*{Presenting the domain}
This provides us with the necessary information to display the grammar coverage in a visualization as seen in \autoref{fig:grammar-coverage}.
...
\project has two ways of implementing an observer: listening to pre-existing event-based systems, or using the meta-level programming techniques of the host language to define meta-event the observer is interested in.
%\on{wtf? registering an observer is not a way to implement it. rewrite and clear up the fuzziness}
...
Object-specific instrumentation is not trivial to achieve in class-based languages like Smalltalk and Java \ab{AspectJ has a perObject keyword I think. Are you focusing on performances?}. 
Classes are deeply rooted in the language interpreter or virtual machine and its performance is tweaked to rely heavily on these constructs.
Moreover, most languages provide a good level of structural reflection to deal with structural elements like classes, method, statements, \etc
But most languages do not provide a standard mechanism to reflect on the dynamic abstractions of the language.
There are typically no abstractions to intercept meta-events such as a message send, a message receive, a state read, \etc.

Albedo has been designed as an evolution of partial behavioral reflection for Smalltalk, which in turn was conceived as an extension of the Reflex model~\cite{Tant03a}.
%\ab{is the following important? I would comment out all the remaining}
%\on{yes, please kill it}
%\ugh{Since Reflex was originally realized with Java, the Albedo approach would be achievable in a more static mainstream language like Java. 
...
\on{funny to have just one subsection. maybe make a separate section?}

Profiling always impacts the performance of the application being analyzed. 
...
\project has two mechanism to observe an application: registering the observer to preexisting event-based systems, or using the meta-level programming techniques of the host language.
...
%In this paper we have presented \project, a framework for defining domain-specific profilers.
Our contributions are the following:
\begin{enumerate}
    \item We have demonstrate the need for domain-specific profilers. We showed how standard profilers only concerned with the source code representation are inadequate for profiling domain-specific problems. We demonstrated this drawback with two use cases.
    \item We stated the requirements domain-specific profilers must fulfill: specifying the domain, capturing domain related events and presenting the necessary information. 
    \item We presented \project a framework for defining domain-specific profilers. We also presented three real-world case-studies showing how \project fulfills the domain-specific profiler requirements.
\end{enumerate}

As future work we plan to ... \jr{@Alex @Lukas any ideas here?}
\ab{What about the following}
\begin{itemize}
\item profile a group of objects, instead a unique one
\item We already presented two visualizations (\autoref{MondrianExample} and \autoref{MondrianProfiler}). The second has has been used for Mondrian, and OB. Is there a way to classify different domain-specific profiler? Are there some common pattern in what has to be profiled? Classification?
\end{itemize}
