...
Traditional execution sampling profilers centers their information on the frames of the execution stack and completely ignore the identify of the object that received the method call and its arguments. As a consequence, it is hard to track down what are the object culprit of the slowness. On the example above, MessageTally says that we spent 30.9\% in \ct{MONode>>displayOn} without saying which node was actually refreshed too often.
...
We identified five distinct properties that domain-specific profilers must fulfill to be effective:
...
\paragraph{domain specific metrics.} \ab{We need to compute some metrics, else no much can be done}
