...
While research on domain-specific languages has made consistent progress in language specification~\cite{Deur00a}, implementation~\cite{Cuad09a}, evolution~\cite{Free06a} and verification~\cite{Kaba08a}, little has been done on profiling. We consider profiling to be the activity to record and analyze program execution. Profiling is essential to analyze transient run-time data that otherwise would be to hard to harvest and compare. Code profilers commonly employ \chg{executing}{execution} sampling as the way to obtain dynamic run-time information. Unfortunately, information extracted \chg{when}{by} regularly sampling the call stack cannot be meaningfully used to profile a high-level domain built on top of the standard language infrastructure.
...
\ins{Specialized domains need specialized profilers.}
...
Standard code profilers did not help to fix this since they are just able to give the share the CPU spent in the method \co{\#displayOn:} of the class \co{MONode} and \co{MOEdge}. The problem has been solved without a standard profiler by identifying \emph{which} nodes and edges were indeed refreshed too often. Knowing the presence of some particular frames on the method call stack (as the majority of code profiler provide) does not \del{satisfactorily} help identify which of the rendered nodes are responsible for a slowdown. The Mondrian domain is defined as a graph of objects, which cannot be easily retrieved from the method call stack and from the source code artifacts.
\on{The paragraph ends in an anti-climax. I would add something like this:}
\ins{A specialized Mondrian profiler would be aware of Mondrian's domain concepts and provide profiling information tailored to the relevant structures.}
...
We identified the following requirements a domain-specific profiler must fulfill: \ugh{\emph{specifying the domain}, \emph{capture domain events}, \emph{effective information presentation}}.
\on{should be: specifying, capturing, presenting}
By fulfilling these requirements, a profiler will be effective at identifying bottlenecks and anomalies in the application code and be able to present these in terms of the \chg{application code}{relevant domain concepts}.
\on{?}
...
In June 2010 a serious performance issue was identified\footnote{\lr{again?} \url{http://bit.ly/g8GjIh} \on{please use a custom URL. bit.ly supports this}} when \chg{performing}{producing} a visualization.
Tracking down the cause of the poor performance was not trivial. We first used the sample-based code \ugh{profiled provided} by Pharo\footnote{\url{http://pharo-project.org/}}.
...
Beside \ugh{Smalltalk profiler execution sampling} \on{I cannot parse this. I suggest to delete the first three words} has been adopted by almost all mainstream profilers (JProfiler\footnote{\url{http://www.ej-technologies.com}}, YourKit\footnote{\url{http://www.yourkit.com}}, xprof~\cite{Gupt92a}, hprof\footnote{\url{http://java.sun.com/developer/technicalArticles/Programming/HPROF.html}}).
...
Getting the coverage of a grammar for \ugh{a set of referential input text} \on{cannot parse --- what does this mean?} is useful to identify unused and duplicated parsing rules.
\on{Maybe you mean this?:}
\ins{It would be useful to establish how much of the grammar is actually exercised by a set of test files in order to identify unused and duplicated parsing rules.}
...
Coverage tools assess the coverage of the application source code by \chg{giving the list of}{listing the} methods involved in an execution. Some tools can even detect the coverage inside methods.
...
\ugh{When in need of analyzing the parsing and production coverage of the Java Grammar parser traditional coverage tools only provide source code related data instead of domain-specific.} \on{puke! The subject of the sentence is "coverage tools", which are not in need of anything! rewrite}
...
\on{How about: \project in a Nutshell?}
...
These observers follow the observer pattern~\cite{Gamm95a} and the profilers are the \ugh{subscribers}.
...
Some observers \ugh{work by means of registering} to existing hooks. Other observers intercept the system using meta-programming, \chg{this}{which} is also known as instrumentation.
...
\begin{description}
    \item[Announcement Observer.] Announcements provide a flexible framework for event handling. The announcement observer \ugh{allows to} hook into applications using this infrastructure and to dispatch them to the profiler.
    \item[Method Observer.] This observer listens to the event when an specific method is invoked for any instance of a class.
    \item[Object Observer.] This observer allows us to trigger \ugh{event} \on{an event? events?} for a single instance of a class.
    \on{wtf? observers don't trigger events, they observe them!}
    ...
    \item[Parser Observer.] \ugh{The parser observer triggers an event} \on{no no no!} whenever a specific production is activated.
...
The performance issue of Mondrian described earlier (\autoref{sec:problem}) is rooted in a complex combination of different aspects of Mondrian rendering: redundancy of metrics calculation and unnecessary \ins{repainting of?} graphical elements \del{repaint}. Theses two aspects were independently addressed, using a dedicated profiler.
\on{Funny. In the intro you just said "without a standard profiler". You should be more precise in the intro.}
...
\begin{itemize}
\item A node may nest other nodes and edges. The \emph{bitmap form cache} avoids rendering inner nodes by taking a snapshot of a bitmap of the \ugh{nesting node}. \on{node nesting? nested node?} Once the bitmap cache is loaded, inner nodes are not rendered any further.
...
A Mondrian visualization may comprise a great number of graphical elements. At each refresh of the Mondrian visualization dictated by the operating system, \ugh{only the elements that need to be refreshed must effectively be.}
\on{duh}
Elements that are outside the window or for which their nesting node has an active bitmap form cache should not be rendered. 
...
\on{not sure what we learn from this case study. the reader gets a bit lost in the details. at least provide a short summary paragraph underlining what we have learned, and how this validates \project. aside from the fact that we have now a domain-specific profile for mondrian, what is cool?}
...
\on{whoops. delete this case study?}
...
\on{weird that this section is so short while the mondrian example is so detailed. I expect a more consistent treatment of each case study}
...
\project has two ways of implementing an observer: registering the observer to pre-existing event-based systems, or using the meta-level programming techniques of the host language.
...
Albedo's meta-objects provide a structural view and a behavioral view. In the context of \project we were mainly interested in behavioral reifications. A behavioral meta-object reifying message sends was used for the message send observer. \ugh{As well as a behavioral meta-object for reifying received message.} \on{not a sentence} State read and write are also supported by behavioral meta-objects thus \project can profile these dynamic events.
...
There are typically no abstractions \chg{for}{to intercept} \on{?} meta-events such as a message send, a message receive, a state read, \etc.
